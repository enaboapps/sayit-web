...
import { parseOpenBoardUpload } from '@/lib/open-board-format/files';
...
export default function OpenBoardImportModal({ isOpen, onClose, onImported }: OpenBoardImportModalProps) {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setIsParsing(true);
    try {
      const parsed = await parseOpenBoardUpload(file);
      setNormalized(normalizeOpenBoardPackage(parsed));
    } catch (err) {
