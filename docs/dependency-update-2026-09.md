...
- Convex deployment validation exposed a shared type importing the browser speech implementation. Move the provider union into the shared types module; run the separate Convex tsconfig in `pnpm typecheck` so CI catches this server/browser boundary. Regenerate Convex helpers with the updated CLI.
